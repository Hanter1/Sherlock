...
Приложение работает только с открытыми HTTP/DNS-источниками и **не** подключается к закрытым базам. Публичный поиск может находить ФИО и следы утечек, уже попавшие в открытый индекс.
